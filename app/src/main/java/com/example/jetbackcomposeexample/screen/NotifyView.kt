...
fun DrawScope.drawNotifyMove(
    ...
    val notifyMoveOffsetY = NUM_NOTIFY - (valueAnim1 + valueAnim2 + valueAnim3)
    for (index in 0 until NUM_NOTIFY) {
        val top =
            heightNotifyPermanent + topNotifyPermanent + (index * hView) + paddingTop - ((3 - notifyMoveOffsetY) * hView)
